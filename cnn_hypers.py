...
def get_bounds():
  return [{'name': 'lr', 'type': 'continuous', 'domain': [0.001, 1]},
          {'name': 'momentum', 'type': 'continuous', 'domain': [0.001, 1]},
          {'name': 'dr1', 'type': 'continuous', 'domain': [0.001, 1]},
...
def evaluate(learning_rate=0.01, momentum=0.9, dropout_rate_1=0.01, dropout_rate_2=0.005, dropout_rate_3=0.005):
    ...
    sgd = SGD(lr=learning_rate, decay=1e-6, momentum=momentum, nesterov=False)
    ...
    # f.close()
    # 
def evaluate_loss(hyperparameters):
    print(hyperparameters[0][0])
    return evaluate(hyperparameters[0][0], hyperparameters[0][1], hyperparameters[0][2], 
        hyperparameters[0][3], hyperparameters[0][4])

if __name__ == '__main__':
    myProblem = BayesianOptimization(evaluate_loss, get_bounds())
